(function() {
  ...
  _.last = function(array, n) {
    if(n === 0) {
      ...
      return n === undefined ? array[array.length - 1] : (n > array.length ? array.slice(0) : array.slice(n - 1, array.length));
    }
  };
  ...
  _.reject = function(collection, test) {
    return _.filter(collection, function(each) {
      return !test(each);
    });
  };
  ...
  _.every = function(collection, iterator) {
    return _.reduce(collection, function(isTrue, item){
      if(iterator === undefined) {
        return item ? isTrue = true : isTrue = false;
      } else if (isTrue) {
        return iterator(item) ? isTrue = true : isTrue = false;
      }
      return isTrue;
    }, true)
  };
  ...
  _.zip = function() {
    ...
    var longestLength = _.reduce(args, function(p, c) {
      return Math.max(p, c.length);
    }, 0);
    _.each(args, function(array) {
      while(array.length < longestLength) {
        array.push(undefined);
      }
    });
    return _.reduce(args, function(acc, curr) {
      _.each(curr, function(currItem, currIndex) {
        acc[currIndex] = acc[currIndex] === undefined ? [] : acc[currIndex];
        acc[currIndex].push(currItem);
      });
      return acc;
    }, []);
  };
